    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<ResponseObj> ForgetPassword(string email, string requestScheme, string requestHost)
        {
            try
            {
                ...
                string link = $"{_privateUrl}/ResetPassword/{HttpUtility.UrlEncode(appUser.Email)}/{HttpUtility.UrlEncode(token)}";
                ...
                _sendEmail.Send("isiriyev@gmail.com", "E-Commerce", appUser.Email, resetPasswordBody, "Reset Password");
        ...
        public async Task<ResponseObj> ProfileUpdate(string id, UserUpdateDto userUpdateDto, string requestScheme, string requestHost)
        {
            try
            {
                ...
                if (!appUser.EmailConfirmed)
                {
                    ...
                    _sendEmail.Send("isiriyev@gmail.com", "E-Commerce", appUser.Email, verificationMessageBody, "Confirm Account");
                    return new ResponseObj
                    {
                        StatusCode = (int)StatusCodes.Status200OK,
